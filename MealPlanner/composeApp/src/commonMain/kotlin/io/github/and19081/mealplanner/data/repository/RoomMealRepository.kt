...
class RoomMealRepository(private val db: MealPlannerDatabase, private val scope: CoroutineScope) :
    MealRepository {

  private val mealDao = db.prePlannedMealDao()

  override val meals: StateFlow<List<PrePlannedMeal>> =
      mealDao
  ...
  override suspend fun upsertMeal(meal: PrePlannedMeal) {
    val mealEntity = PrePlannedMealEntity(id = meal.id, name = meal.name, mealType = meal.mealType)

    val recipes = meal.recipes.map { recipeId -> MealRecipeEntity(meal.id, recipeId) }

    val independentIngredients =
        meal.independentIngredients.map { mi ->
          MealIndependentIngredientEntity(
    ...
    mealDao.upsertMealWithDetails(mealEntity, recipes, independentIngredients)
  }

  override suspend fun removeMeal(id: Uuid) {
    val entity = mealDao.getById(id)

...
data class RecipeExecutionState(
  ...
  val scaleFactor: Double
    get() = if (recipe.recipeInfo.servings > 0) targetServings / recipe.recipeInfo.servings else 1.0

  val totalSteps: Int
    get() = recipe.recipeInfo.instructions.size

  val isOnOverview: Boolean
      ...
      recipe.recipeInfo.requirementGroups
          ...
          .map { req ->
            ScaledRequirement(
                ...
                ingredientName = req.measurement.foodItemId?.let { allItemNames[it] } ?: "Unknown",
                unitAbbreviation = req.measurement.unitId?.let { allUnitAbbr[it] } ?: "",
            )
          }
}

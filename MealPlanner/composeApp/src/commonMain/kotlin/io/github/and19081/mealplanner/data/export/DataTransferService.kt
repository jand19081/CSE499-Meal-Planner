...
class DataTransferService(private val database: MealPlannerDatabase) {
    ...
    suspend fun exportData(): String {
        val payload = BackupPayloadV1(
            ...
            leftoverComponents = database.foodItemDao().getAllLeftoverComponents().map { it.toDtoV1() },
            ...
            recipeRequirementGroups = database.foodItemDao().getAllRequirementGroups().map { it.toDtoV1() },
        ...
        return runCatching {
            ...
            if (version == 1) {
                ...
                database.useWriterConnection {
                    ...
                    database.foodItemDao().upsertAllPurchasable(backup.purchasableComponents.map { it.toEntity() })
                    database.foodItemDao().upsertAllRecipeComponents(backup.recipeComponents.map { it.toEntity() })
                    database.foodItemDao().upsertAllLeftoverComponents(backup.leftoverComponents.map { it.toEntity() })
                    database.foodItemDao().upsertAllInstructions(backup.recipeInstructions.map { it.toEntity() })
                    ...
                    database.foodItemDao().upsertAllConversions(backup.unitConversionBridges.map { it.toEntity() })
                    database.packageOptionDao().upsertAll(backup.packageOptions.map { it.toEntity() })
                    database.foodItemDao().upsertAllRequirementGroups(backup.recipeRequirementGroups.map { it.toEntity() })
                    database.foodItemDao().upsertAllRequirements(backup.recipeRequirements.map { it.toEntity() })

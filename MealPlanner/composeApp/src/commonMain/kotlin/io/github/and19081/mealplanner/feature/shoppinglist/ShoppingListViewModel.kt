...
class ShoppingListViewModel(
      ...
      combine(coreDataFlow, userDataFlow) { core, user ->
        ...
        fun addRecursive(itemId: Uuid, qty: Double, unitId: Uuid?, multiplier: Double) {
          ...
          if (item.isRecipe()) {
            ...
            val scale =
                if (recipeInfo.servings > 0) (qty * multiplier) / recipeInfo.servings else 1.0
        ...
        entries
            .filter { !it.isConsumed && it.date >= today }
            .forEach { entry ->
            ...
            .forEach { customItem ->
              ...
              if (linkedFoodItem != null) {
                ...
                if (linkedFoodItem.isRecipe()) {
                  ...
                  val unit =
                      unitsMap[customItem.measurement.unitId ?: linkedFoodItem.preferredUnitId]
                  if (unit != null) {
                    val (baseQty, _) =
                        UnitConverter.toStandard(customItem.measurement.quantity, unit, unitsMap)
                    val types =
                        grossRequirementsByType.getOrPut(linkedFoodItem.id) { mutableMapOf() }
                    types[unit.type] = (types[unit.type] ?: 0.0) + baseQty
                  }
                }
              } else {
        ...
        grossRequirements.forEach { (itemId, reqQty) ->
          ...
          val grossRequirementsTargetType =
              preferredType
                  ?: (grossRequirementsByType[itemId]?.maxBy { it.value }?.key ?: UnitType.Count)
          ...
          val ownedQty =
              if (pantryItem != null) {
                ...
                if (pUnit != null) {
                  val targetBaseUnitId =
                      when (grossRequirementsTargetType) {
                        UnitType.Mass -> SystemUnits.Gram.id
                        UnitType.Volume -> SystemUnits.Ml.id
                        UnitType.Count -> SystemUnits.Each.id
                        else -> null
        ...
        netRequirements.forEach { (itemId, baseQtyNeeded) ->
          ...
          if (bestOption != null && bestOption.quantity > 0) {
            ...
            if (optUnit != null) {
              val (baseOptionQty, _) =
                  UnitConverter.toStandard(bestOption.quantity, optUnit, unitsMap)
              if (baseOptionQty > 0) {
  ...
  fun skipPriceUpdate() {
    _receiptDraft.update { it?.copy(userEnteredTotalCents = null) }
  }
  ...
  fun commitTransaction(transaction: KitchenTransaction) {
    viewModelScope.launch {
      ...
      val customItemsInCart =
          uiState.value.sections.flatMap { it.items }.filter { it.isInCart && it.isCustom }
      ...
      val processedIds =
          transaction.changes.mapNotNull { it.measurement.foodItemId }.toSet() +
              customItemsInCart.map { it.id }.toSet()
  ...
  fun submitReceiptTotal(
    ...
    _receiptDraft.update { it?.copy(userEnteredTotalCents = actualTotalCents) }
    if (forcePriceReview) {
        ...
        items
            ...
            .map { item ->
              ...
              ReceiptLineItemDraft(
                  ...
                  name = item.name,
                  quantity = item.quantity,
                  unitAbbreviation = unit?.abbreviation ?: item.unit,
                  unitId = unit?.id,
                  priceCents = item.priceCents.toLong().takeIf { it > 0 } ?: 0L,
              )
            }

    _receiptDraft.value =
        ShoppingReceiptDraft(
            storeId = storeId,
            storeName = storeName,
            lineItems = lineItems,
          ...
          lineItems =
              draft.lineItems.map { item ->
                if (item.foodItemId == foodItemId) {
                  item.copy(priceCents = priceCents.toLong())
                } else {
                  item
                }
              }
  ...
  fun recordActualTotal(actualTotalCents: Int) {
    _receiptDraft.update { it?.copy(userEnteredTotalCents = actualTotalCents.toLong()) }
  }
    ...
    viewModelScope.launch {
      ...
      val lineItems =
          draft.lineItems.map { draftItem ->
            ReceiptLineItem(
                ...
                customName = null,
                pricePaidCents = draftItem.priceCents.toInt(),
            )
          }
      ...
      val receipt =
          ReceiptHistory(
              ...
              projectedTotalCents = draft.computedTotalCents.toInt(),
              actualTotalCents = (draft.userEnteredTotalCents ?: draft.computedTotalCents).toInt(),
              taxPaidCents = draft.taxCents.toInt(),
              lineItems = lineItems,
          )
        ...
        if (unit != null) {
          val converted =
              UnitConverter.convert(item.quantity, unit.id, unit.id, allUnits.associateBy { it.id })
                  ?: item.quantity
          val (baseQtyToAdd, _) =
              UnitConverter.toStandard(converted, unit, allUnits.associateBy { it.id })
  ...
  fun addCustomItem(name: String, qty: Double, unitId: Uuid, isPantry: Boolean = true) {
    viewModelScope.launch {
      shoppingListItemRepository.addItem(
          ShoppingListItem(
              customName = name,
              measurement = ItemMeasurement(foodItemId = Uuid.NIL, unitId = unitId, quantity = qty),

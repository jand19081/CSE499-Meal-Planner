                        ...
                        Column(
                            ...
                            requirementGroups.flatMap { it.requirements }.forEach { req ->
                                ...
                                Row(verticalAlignment = Alignment.CenterVertically) {
                                    ...
                                    Icon(
                                        if (totalInStock >= req.quantity) Icons.Default.CheckCircle else if (totalInStock > 0) Icons.Default.RemoveCircle else Icons.Default.Warning,
                                        null,
                                        tint = statusColor,
                                    ...
                                    Text(
                                        ...
                                        } / ${req.quantity} ${allUnits.find { it.id == req.unitId }?.abbreviation ?: ""}",
                                        style = MaterialTheme.typography.bodySmall,
                                        color = statusColor,
                                    )
                                }
...
fun RecipeIngredientsEditor(
  ...
  Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
      ...
      Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant.copy(alpha = 0.5f))) {
        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
          FlowRow(
            ...
            Text("${groupIndex + 1}", style = MaterialTheme.typography.titleSmall)
                ...
                Box(modifier = Modifier.widthIn(min = 120.dp).fillMaxWidth(0.6f)) {
                    SearchableDropdown(
                        ...
                        onOptionSelected = { option ->
                            ...
                            val cleanName =
                                if (isRecipe) option.removePrefix("[Recipe] ") else option
                            val found =
                                allItems.find { it.name == cleanName && it.isRecipe == isRecipe }
                            onUpdate(requirementGroups.mapIndexed { i, g ->
                                if (i == groupIndex) g.copy(requirements = g.requirements.mapIndexed { ri, r ->
                                    if (ri == reqIndex) r.copy(
                                        foodItemId = found?.id ?: Uuid.NIL
                                    ) else r
                                }) else g
                            })
                        },
          ...
          TextButton(onClick = {
            onUpdate(requirementGroups.mapIndexed { i, g ->
                if (i == groupIndex) g.copy(requirements = g.requirements + FoodItemRequirement(
                    foodItemId = Uuid.NIL,
                    quantity = 0.0,
                    isPrimary = false
                )
    ...
    Button(onClick = { onUpdate(requirementGroups + FoodItemRequirementGroup(
        requirements = listOf(

...
fun RecipesView(
  ...
  val onRecipeClick: (FoodItem) -> Unit = { viewModel.startEditing(it) }

  val onAddClick: () -> Unit = { viewModel.startEditing(null, uiState.searchQuery) }

  val onDismissDetail: () -> Unit = { viewModel.clearDraft() }
        ...
        Column {
          ...
          Row(modifier = Modifier.padding(horizontal = 16.dp, vertical = 4.dp)) {
            FilterChip(
                ...
                leadingIcon =
                    if (uiState.isCanMakeNowFilterActive) {
...
fun RecipeForm(
  ...
  MpDetailScaffold(
      title =
          if (uiState.allRecipes.any { it.id == draftState.id }) "Edit Recipe" else "New Recipe",
      onClose = onDismiss,
      onSave = {
        val finalRecipe =
            Recipe(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
          ...
          Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            MpNumericStepper(
                ...
                onValueChange = { newServings ->
                  onUpdateDraft { it.copy(servingsStr = newServings.toString()) }
                },
                label = "Servings",
          ...
          Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            MpNumericStepper(
                ...
                onValueChange = { newPrep ->
                  onUpdateDraft { it.copy(prepTimeStr = newPrep.toInt().toString()) }
                },
                label = "Prep (min)",
                modifier = Modifier.weight(1f),
                step = 5.0,
            ...
            MpNumericStepper(
                ...
                onValueChange = { newCook ->
                  onUpdateDraft { it.copy(cookTimeStr = newCook.toInt().toString()) }
                },
                label = "Cook (min)",
                modifier = Modifier.weight(1f),
                step = 5.0,
            ...
            Column(
              ...
              stockWarnings.forEach { warning ->
                Row(verticalAlignment = Alignment.CenterVertically) {
                  ...
                  Icon(
                      if (warning.isSufficient) Icons.Default.CheckCircle
                      else if (warning.isPartial) Icons.Default.RemoveCircle
                  ...
                  Text(
                      text =
                          "${warning.itemName}: ${String.format("%.1f", warning.totalInStock)} / ${warning.requiredQuantity} ${warning.unitAbbreviation}",
                      style = MaterialTheme.typography.bodySmall,
                      color = statusColor,
                  )
                }
              ...
              if (stockWarnings.isEmpty()) {
                Text(
                    "Add ingredients to check stock.",
                    style = MaterialTheme.typography.bodySmall,
    ...
    if (instructions.isEmpty()) {
      ...
      instructions.forEachIndexed { index, step ->
        InstructionRow(
            ...
            onRemove = { onUpdate(instructions.toMutableList().apply { removeAt(index) }) },
        )
        HorizontalDivider()
      }
    }
...
@Composable
fun InstructionRow(step: String, index: Int, onUpdateStep: (String) -> Unit, onRemove: () -> Unit) {
                  ...
                  onUpdate(
                      requirementGroups.mapIndexed { i, g ->
                        if (i == groupIndex) {
                          g.copy(
                              requirements =
                                  g.requirements.mapIndexed { ri, r ->
...
fun IngredientRequirementRow(
  ...
  val selectedItemName =
      (if (selectedItem?.isRecipe() == true || selectedItem?.isMeal() == true) "[Recipe] "
  ...
  if (isEditing) {
      ...
      FlowRow(
        ...
        SearchableDropdown(
            ...
            onOptionSelected = { option ->
              ...
              val found = allItems.find {
                it.name == cleanName && (it.isRecipe() || it.isMeal()) == isRecipe
              }
              onUpdateReq(
                  req.copy(measurement = req.measurement.copy(foodItemId = found?.id ?: Uuid.NIL))
              )
            },
            onAddOption = { name ->
        ...
        MpOutlinedTextField(
            ...
            onValueChange = { newText ->
              ...
              onUpdateReq(
                  req.copy(
                      measurement = req.measurement.copy(quantity = newText.toDoubleOrNull() ?: 0.0)
                  )
              )
            },
            label = { Text("Qty") },
    ...
    Row(
      ...
      val qtyString =
          if (req.measurement.quantity > 0.0) req.measurement.quantity.toString() else ""
      val displayText =
          listOf(selectedItemName, ":", qtyString, selectedUnitName)

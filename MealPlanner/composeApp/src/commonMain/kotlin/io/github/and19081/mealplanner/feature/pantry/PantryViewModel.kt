...
class PantryViewModel(
    ...
    fun consumeItem(pantryItemId: Uuid, amountConsumed: Double) {
        viewModelScope.launch {
            val pantryItem = pantryRepository.pantryItems.value.find { it.id == pantryItemId } ?: return@launch
    ...
    fun updateLeftoverQuantity(id: Uuid, servings: Double) {
        viewModelScope.launch {
            ...
            val updatedLeftoverState = when {
                servings <= 0 -> LeftoverState.Deleted
                servings < existing.recipeInfo?.servings ?: 0 -> LeftoverState.Remaining(
                    remainingServings = servings,
                    originalServings = existing.recipeInfo?.servings ?: 0,
                    dateAdded = info.dateAdded,
                    expirationDate = info.expirationDate
                )
            ...
            when (updatedLeftoverState) {
                ...
                LeftoverState.Consumed -> { /* No action needed - item is fully consumed */
                }
            }
        }
    }
